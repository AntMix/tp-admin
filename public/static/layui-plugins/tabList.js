...
let OpenTabMenuFun = function ($, callback) {
   var tabMenu = sessionStorage.getItem(window.config.tabMenuName);//已经打开的tab页面
   if (tabMenu) {
...
/**删除缓存中的tabMenu**/
let removeTabMenu = function (tabList, callback) {
   sessionStorage.removeItem(window.config.tabMenuName);
   sessionStorage.removeItem(window.config.layIdName);
   if (typeof callback == "function") {
      callback(tabList);
   }
};
...
function saveTabMenuFun($) {
   ...
   sessionStorage.setItem(window.config.tabMenuName, tabMenu);
}
...
layui.define(["element", "jquery"], function (exports) {
   ...
   tabList = function () {
      this.tabConfig = {
         ...
         tabFilter: "tab-list", //添加窗口的filter
         url: "", //获取菜单的接口地址
    ...
    * @returns {tabList}
    */
   tabList.prototype.init = function (option) {
   ...
   //生成左侧菜单
   var temp = "";
   tabList.prototype.navBar = function (strData) {
      var data;
   ...
   function createMenu(data, tabID) {
      if (data.children != undefined && data.children.length > 0) {
         ...
         for (var i = 0; i < data.children.length; i++) {
            temp += '<dd>';
            var lay_id = tabID + "-" + data.children[i].id;
    ...
    * @param ele 当前tab
    */
   tabList.prototype.positionTab = function (contEle, ele) {

      var superEle = $(".tab-list");//父级元素
   ...
   // 点击导航页的操作
   tabList.prototype.tab = function (e) {
      var that = this;
      ...
      element.on("tab({0})".format(filter), function (data) {
         ...
         sessionStorage.setItem(window.config.layIdName, this.getAttribute('lay-id'));
         var elSuper = $(".tab-list"),//视窗元素
            elMove = $(".tab-list-title"),//移动的元素
            elTabs = $(".tab-list-title li"),//所有已存在的tab集合
   ...
   tabList.prototype.tabDelete = function () {
      var that = this;
      var filter = this.tabConfig.tabFilter;
   ...
   //删除缓存中的tab
   tabList.prototype.removeTabStorage = function (callback) {
      removeTabMenu(this, callback);
   };

   //添加tab页
   tabList.prototype.tabAdd = function (_thisa) {
      var that = this;
      ...
      var thatTabNum = $('.tab-list-title > li').length;//当前已经打开的选项卡的数量
      var iframes = $(".tab-list-content iframe");
      ...
      if (_this.attr("target") == "_blank") {
         ...
         for (var i = 0; i < thatTabNum; i++) {
            if ($('.tab-list-title > li').eq(i).attr('lay-id') == tabId) {
   ...
   tabList.prototype.render = function (callback, reloadTab = true) {
      ...
      if (_this.tabConfig.url) {
         $.get(_this.tabConfig.url, function (res) {
            ...
            var lay_id = sessionStorage.getItem(window.config.layIdName);
    ...
    * 3：代表关闭所有标签页
    */
   tabList.prototype.tabClose = function (num) {
      num = num || 1;
      num = num * 1;//强制转换成数字
      var that = this;
      let openTabs = $('.tab-list-title > li strong[lay-id]').not('strong[is-close=false]'),//获取已打开的tab元素(除开不会被关闭的导航)
         thatLayID = $('.tab-list-title > li.layui-this strong[lay-id]').not('strong[is-close=false]').attr("lay-id") || '';//获取当前打开的tab元素ID(除开不会被关闭的导航)
    ...
    *      为leftmax:跑到最左边
    */
   tabList.prototype.navMove = function (moveId, that) {
      var superEle = $(".tab-list"),//父级元素
         contEle = $(".tab-list-title"),//tab内容存放的父元素
         superWidth = parseInt(superEle.width()) - 40 * 3,//可显示的宽度
   ...
   exports("tabList", function (option) {
      if (parent.tabListObj) {
         return parent.tabListObj;
      } else {
         return new tabList().init(option);
      }
   });
